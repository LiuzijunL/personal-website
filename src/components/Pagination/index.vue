<template>
	<div class="pagination_box" v-if="totalPage >= 1">
		<ul id="pagination">
		    <li class="bothEnds firstPage" @click="switchPage(1)">首页</li>
		    ...
		    <template v-if="totalPage >= 8">
			        ...
			        <template v-for="n in 5">
			            <li v-if="currentPage == 4" :key="n" v-bind:class="{'active': n===currentPage}" v-on:click="switchPage(n)">
			                {{n}}
			            </li>
			        </template>
			        <template v-for="n in 4">
			            <template v-if="currentPage < 4">
			                <li v-bind:class="{'active': n===currentPage}" :key="n" v-on:click="switchPage(n)">
			                    {{n}}
			                </li>
			            </template>
			            <template v-if="currentPage > 4">
			                <li v-if="currentPage+4 <= totalPage" :key="n" v-bind:class="{'active': n+currentPage-2===currentPage}" v-on:click="switchPage(n+currentPage-2)">
			                    {{n+currentPage-2}}
			                </li>
			            </template>
			        </template>
			        <template v-for="n in 4">
			            <li v-if="currentPage+3 > totalPage" :key="n" v-bind:class="{'active': totalPage-4+n===currentPage}" v-on:click="switchPage(totalPage-4+n)">
			                {{totalPage-4+n}}
			            </li>
			        </template>
			        <template v-for="n in 5">
			            <li v-if="currentPage+3 == totalPage" :key="n" v-bind:class="{'active': totalPage-5+n===currentPage}" v-on:click="switchPage(totalPage-5+n)">
			                {{totalPage-5+n}}
			            </li>
